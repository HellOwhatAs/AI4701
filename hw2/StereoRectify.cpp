...
static void icvGetRectangles(const CvMat* cameraMatrix, const CvMat* distCoeffs,
    ...
    undistortPoints(cvarrToMat(_pts), cvarrToMat(_pts), cvarrToMat(cameraMatrix), cvarrToMat(distCoeffs), cvarrToMat(R), cvarrToMat(newCameraMatrix));
    ...
    CvRect* roi1, CvRect* roi2)
{
    ...
    for (k = 0; k < 2; k++)
    {
        ...
        _a_tmp[0][2] = 0.0;
        _a_tmp[1][2] = 0.0;
        projectPoints(cvarrToMat(&pts_3), cvarrToMat(k == 0 ? _R1 : _R2), cvarrToMat(&Z), cvarrToMat(&A_tmp), 0, cvarrToMat(&pts));
...
int main() {
    ...
    Mat cameraMatrixLeft = (Mat_<double>(3, 3) << 717.67713222, 0., 524.32800179, 0., 728.74457604, 285.48227469, 0., 0., 1.);
    Mat cameraMatrixRight = (Mat_<double>(3, 3) << 721.88057791, 0., 513.97875605, 0., 732.52238649, 293.09503537, 0., 0., 1.);
    Mat distCoeffsLeft = (Mat_<double>(1, 5) << 0.03202285, -0.17513817, 0.00090401, 0.00048527, 0.19902392);
    Mat distCoeffsRight = (Mat_<double>(1, 5) << 0.00878113, -0.12596223, -0.00143747, 0.00142956, 0.18402501);

    Mat R = (Mat_<double>(3, 3) << 0.99937181, 0.00763664, -0.03460732, -0.00856809, 0.99960284, -0.02684689, 0.03438855, 0.02712655, 0.99904033);
    Mat T = (Mat_<double>(3, 1) << 189.73626102, -4.52649875, 4.79116498);

    Mat R1, R2, P1, P2, Q;
    stereoRectify(cameraMatrixLeft, distCoeffsLeft, cameraMatrixRight, distCoeffsRight, imgLeft.size(), R, T, R1, R2, P1, P2, Q);

    Mat mapLeftX, mapLeftY, mapRightX, mapRightY;
    initUndistortRectifyMap(cameraMatrixLeft, distCoeffsLeft, R1, P1, imgLeft.size(), CV_32FC1, mapLeftX, mapLeftY);
    initUndistortRectifyMap(cameraMatrixRight, distCoeffsRight, R2, P2, imgRight.size(), CV_32FC1, mapRightX, mapRightY);
