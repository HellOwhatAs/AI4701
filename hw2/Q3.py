...
def getDirectRectifications(A1, A2, RT1, RT2, dims1, dims2):
    ...
    if np.all(np.equal(F/F[2,1], np.array([[0,0,0],[0,0,-1],[0,1,0]]))): w1 = w2 = np.array([0,0,1])
        ...
        if np.all(np.equal(RT1[:,:3], RT2[:,:3])) and np.all(np.equal(A1, A2)) and np.all(np.equal(P1, P2)) and np.all(np.equal(Pc1, Pc2)): sol = [-m[0]/m[1]]
        else:
            m += [C2[1,2]/C2[1,1], C2[1,1]/C1[1,1], M2[1,2]*C2[1,2] - M2[2,2]*C2[1,1], M2[1,1]*C2[1,2] - M2[1,2]*C2[1,1], C1[1,2]/C1[1,1], 1/(C2[1,1]/C1[1,1])]
        ...
        w1, w2 = ((tmpfunc := lambda ss: (((tmp := (Rnew := np.array([
            ...
            np.linalg.inv(A1).dot(np.array([0,ss,1])) - RT1[:,3])) - ((p1w + np.linalg.inv(RT2[:,:3]).dot(RT2[:,3])
...
if __name__ == "__main__":
    ...
    Rectify1, Rectify2 = getDirectRectifications(A1, A2, RT1, RT2, dims1, dims2)
    ...
    scaleX, scaleY = flipX * dims1[0] / (maxX2 - minX2) if maxX2 - minX2 > maxX1 - minX1 else flipX * dims1[0]/(maxX1 - minX1), flipY * dims1[1] / (maxY - minY)
    Fit = np.array([
    ...
    rectImgs = cv2.line(rectImgs, (0,int((dims1[1]-1)/2)), (2*dims1[0]-1,int((dims1[1]-1)/2)), color=(0,0,255), thickness=1)
